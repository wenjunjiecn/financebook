...
const accountTypeConfig: Record<Account['type'], { label: string; icon: React.FC<{ className?: string }>; bg: string; text: string }> = {
  checking: { label: '银行账户', icon: Landmark, bg: 'bg-blue-50', text: 'text-blue-500' },
  credit: { label: '信用卡', icon: CreditCard, bg: 'bg-violet-50', text: 'text-violet-500' },
  wechat: { label: '微信', icon: Smartphone, bg: 'bg-emerald-50', text: 'text-emerald-500' },
  alipay: { label: '支付宝', icon: Smartphone, bg: 'bg-sky-50', text: 'text-sky-500' },
  cash: { label: '现金', icon: Coins, bg: 'bg-amber-50', text: 'text-amber-500' },
}
    ...
    <div className="p-6 h-full overflow-y-auto">
      <div className="max-w-[900px] mx-auto space-y-4">
        ...
        <div className="flex items-end justify-between animate-fade-in">
          <div>
            <h2 className="text-[16px] font-semibold text-gray-900">账户管理</h2>
            <p className="text-[11px] text-gray-400 mt-0.5">管理微信、支付宝、银行卡及现金账户</p>
          </div>
          <div className="card px-4 py-2 text-right">
            <span className="text-[10px] text-gray-400 uppercase tracking-wide block">总资产</span>
            <span className="text-[17px] font-semibold tabular-nums text-gray-900">{formatCentsToYuan(totalBalance)}</span>
          </div>
        </div>
        ...
        {accounts.length === 0 ? (
          <div className="card p-12 text-center animate-fade-in">
            ...
            <p className="text-[13px] font-medium text-gray-500">还没有任何账户</p>
            <p className="text-[11px] text-gray-400 mt-1">在下方添加你的第一个账户</p>
          </div>
        ) : (
              ...
              return (
                <div key={acc.id} className="card card-hover p-4 animate-slide-up" style={{ animationDelay: `${idx * 50}ms` }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[10px] text-gray-400 uppercase tracking-wide">{config.label}</span>
                  </div>
                  <h4 className="font-medium text-gray-900 text-[13px] mb-2 truncate">{acc.name}</h4>
                  <div className="pt-2 border-t border-gray-50 flex items-center justify-between">
                    <span className="text-[11px] text-gray-400">余额</span>
                    <span className="font-mono text-[15px] font-semibold text-gray-900 tabular-nums">{formatCentsToYuan(acc.balance)}</span>
                  </div>
                </div>
              )
            })}
        ...
        <div className="card p-4 max-w-md animate-slide-up">
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            <div>
              <label className="text-[11px] text-gray-400 block mb-1">账户名称</label>
            ...
            <div>
              <label className="text-[11px] text-gray-400 block mb-1">账户类型</label>
                  ...
                  return (
                    <button key={t} type="button" onClick={() => setType(t)}
                      className={`p-2 rounded-md flex flex-col items-center gap-1 transition-all border ${
                        isActive ? `${config.bg} ${config.text} border-current` : 'bg-gray-50 text-gray-400 border-transparent hover:bg-gray-100'
                      }`}>
