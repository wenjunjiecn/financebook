    ...
    <div className="p-6 h-full overflow-y-auto">
      <div className="max-w-[900px] mx-auto space-y-4">
        ...
        <div className="animate-fade-in">
          <h2 className="text-[16px] font-semibold text-gray-900">账单导入</h2>
          <p className="text-[11px] text-gray-400 mt-0.5">支持微信/支付宝 CSV、通用 CSV 映射，以及图片识别</p>
        </div>
            ...
            return (
              <button key={mode.id} onClick={() => setActiveImportMode(mode.id)}
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-[12px] font-medium ${isActive ? 'text-gray-900' : 'text-gray-600'}`}>{mode.label}</span>
                </div>
                <p className={`text-[10px] ${isActive ? 'text-blue-600' : 'text-gray-400'}`}>{mode.desc}</p>
              </button>
            )
          })}
        </div>
        ...
        {activeImportMode === 'generic' && (
          <div className="card p-4 space-y-3 animate-scale-in">
            <div className="flex items-center justify-between">
              <h3 className="text-[13px] font-medium text-gray-900">CSV 列字段映射</h3>
              {templates.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-gray-400">已有模板</span>
            ...
            <div className="grid grid-cols-5 gap-2">
              {[{ key: 'dateColIndex', label: '日期列' }, { key: 'payeeColIndex', label: '商户列' }, { key: 'amountColIndex', label: '金额列' }, { key: 'typeColIndex', label: '类型列' }, { key: 'notesColIndex', label: '备注列' }].map((field) => (
                <div key={field.key}>
                  <label className="text-[11px] text-gray-400 block mb-1">{field.label}</label>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-50">
              ...
              <button onClick={handleSaveTemplate} className="btn-secondary px-3 py-1.5 rounded-md text-[12px] flex items-center gap-1.5"><Save className="w-3.5 h-3.5 text-gray-500" />保存</button>
            </div>
          </div>
        )}
        ...
        <div onDragOver={(e) => { e.preventDefault(); setIsDragOver(true) }} onDragLeave={() => setIsDragOver(false)} onDrop={handleDrop} onClick={handleFileSelect}
          className={`rounded-xl p-10 flex flex-col items-center justify-center cursor-pointer transition-all text-center border-2 border-dashed ${
            isDragOver ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'
          }`}>
          <div className={`p-3 rounded-xl mb-3 ${activeImportMode === 'image' ? 'bg-amber-50' : 'bg-blue-50'}`}>
            {activeImportMode === 'image' ? <ImageIcon className="w-6 h-6 text-amber-500" /> : <Upload className="w-6 h-6 text-blue-500" />}
          </div>
          <p className="text-[13px] font-medium text-gray-700">{activeImportMode === 'image' ? '拖入小票/截图，或点击选择' : '点击选择文件，或拖入此区域'}</p>
          <p className="text-[11px] text-gray-400 mt-1">{activeImportMode === 'image' ? '支持 JPG / PNG 票据图片' : '支持 UTF-8 / GBK 编码 CSV'}</p>
        </div>

        {/* AI Loading */}
        {aiLoading && (
          <div className="card p-4 flex items-center gap-3 animate-scale-in" style={{ borderColor: '#fcd34d' }}>
            <div className="p-2 rounded-lg bg-amber-50"><Sparkles className="w-4 h-4 text-amber-500 animate-spin" /></div>
            <div className="flex-1">
              <p className="text-[12px] font-medium text-amber-700">图片识别中</p>
              <p className="text-[11px] text-gray-400 mt-0.5">正在解析票据信息...</p>
            </div>
          </div>
        )}
        ...
        {imagePreview && (
          <div className="card p-3 flex items-center gap-4 animate-scale-in">
            <img src={imagePreview} alt="preview" className="w-14 h-14 object-cover rounded-md border border-gray-200" />
            <div className="text-[12px]">
              <span className="font-medium text-gray-900 block">已加载预览图片</span>
              <span className="text-gray-400 text-[11px] mt-0.5 block">识别结果将在下方呈现，确认后入账</span>
            </div>
          </div>
        )}
        ...
        {candidates.length > 0 && (
          <div className="space-y-2 animate-fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h3 className="text-[13px] font-medium text-gray-900">待确认候选</h3>
                <span className="badge bg-gray-100 text-gray-500 tabular-nums">{candidates.length} 笔</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="card overflow-hidden">
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="w-full text-left text-[12px]">
                  <thead className="sticky top-0 bg-gray-50 text-gray-500 uppercase border-b border-gray-100">
                    <tr>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {candidates.map((c) => (
                      <tr key={c.tempId} className={`hover:bg-gray-50 transition-colors ${c.status === 'duplicate' ? 'bg-amber-50/40' : ''}`}>
                        ...
                        <td className="p-2.5">
                          {c.status === 'duplicate' ? (
                            <span className="badge bg-amber-50 text-amber-600" title={c.duplicateMatchReason}><AlertTriangle className="w-2.5 h-2.5" />重复</span>
                          ) : c.isRefund ? (
                            <span className="badge bg-blue-50 text-blue-600"><RotateCcw className="w-2.5 h-2.5" />退款</span>
                          ) : (
                            <span className="badge bg-blue-50 text-blue-600"><CheckCircle2 className="w-2.5 h-2.5" />正常</span>
                          )}
                        </td>
                        <td className="p-2.5 font-mono text-gray-600 tabular-nums text-[11px]">{c.date}</td>
                        <td className="p-2.5">
                          <input type="text" value={c.payee} onChange={(e) => updateCandidate(c.tempId, { payee: e.target.value })}
                            className="bg-transparent border-b border-transparent focus:border-blue-400 text-gray-900 text-[12px] px-1 py-0.5 focus:outline-none w-full transition-colors" />
                        </td>
                        <td className="p-2.5">
                        ...
                        <td className="p-2.5 text-right font-mono font-medium text-blue-600 tabular-nums">{formatCentsToYuan(c.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
