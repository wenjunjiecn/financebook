...
const toastConfig: Record<ToastType, { icon: React.FC<{ className?: string }>; color: string; bg: string }> = {
  ...
  loading: { icon: Loader2, color: 'text-amber-500', bg: 'bg-amber-50' },
}
    ...
    <ToastContext.Provider value={{ toast, success, error, info, loading, dismiss, confirm }}>
            ...
            <div key={t.id} className="pointer-events-auto animate-slide-in-right min-w-[280px] max-w-[380px]">
              <div className="card p-3 flex items-start gap-2.5 shadow-md">
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className="text-[13px] font-medium text-gray-900 leading-tight">{t.title}</p>
                  {t.message && <p className="text-[11px] text-gray-500 mt-0.5 leading-relaxed break-words">{t.message}</p>}
                </div>
                {t.type !== 'loading' && (
                  <button onClick={() => dismiss(t.id)} className="text-gray-300 hover:text-gray-600 transition-colors shrink-0 -mr-0.5 -mt-0.5 p-1">
                    <X className="w-3 h-3" />
                  </button>
                )}
              </div>
      ...
      {confirmState && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center p-4 animate-fade-in app-no-drag">
          <div className="absolute inset-0 bg-black/20 backdrop-blur-[2px]" onClick={() => handleConfirm(false)} />
          <div className="relative card w-full max-w-sm p-5 animate-scale-in shadow-xl">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1 pt-0.5">
                <h3 className="text-[14px] font-medium text-gray-900">{confirmState.title}</h3>
                {confirmState.message && <p className="text-[12px] text-gray-500 mt-1 leading-relaxed">{confirmState.message}</p>}
              </div>
            </div>
