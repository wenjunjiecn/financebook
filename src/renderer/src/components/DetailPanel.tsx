...
export const DetailPanel: React.FC = () => {
  ...
  return (
    <aside className="w-[340px] bg-white border-l border-gray-100 flex flex-col shrink-0 animate-slide-in-right overflow-hidden">
      {/* Header */}
      <div className="h-11 flex items-center justify-between px-4 border-b border-gray-50 shrink-0">
        <span className="text-[12px] font-medium text-gray-600">交易详情</span>
        <button onClick={() => setSelectedTransaction(null)} className="btn-ghost p-1 rounded-md text-gray-400 hover:text-gray-700">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Amount */}
        <div>
          <div className="text-[11px] text-gray-400 mb-1">金额</div>
          <div className={`text-[28px] font-semibold tabular-nums ${
            txType === 'expense' ? 'text-gray-900' : txType === 'income' ? 'text-emerald-600' : 'text-blue-600'
          }`}>
          ...
          <div className="mt-1 flex items-center gap-1.5">
            ...
            <span className="badge bg-gray-100 text-gray-500">{draft.source === 'wechat' ? '微信' : draft.source === 'alipay' ? '支付宝' : draft.source === 'manual' ? '手动' : draft.source}</span>
          </div>
        </div>
        ...
        {/* Type */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block">类型</label>
          <div className="grid grid-cols-3 gap-1">
            {(['expense', 'income', 'transfer'] as TransactionType[]).map((t) => (
              <button key={t} onClick={() => update({ type: t })}
                className={`py-1.5 rounded-md text-[11px] transition-colors ${
                  draft.type === t ? 'bg-blue-500 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                }`}>
        ...
        {/* Payee */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block flex items-center gap-1"><FileText className="w-3 h-3" />商户</label>
        ...
        {/* Amount input */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block flex items-center gap-1"><DollarSign className="w-3 h-3" />金额（元）</label>
        ...
        {/* Date */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block flex items-center gap-1"><Calendar className="w-3 h-3" />日期时间</label>
        ...
        {/* Category */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block flex items-center gap-1"><Tag className="w-3 h-3" />分类</label>
        ...
        {/* Account */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block flex items-center gap-1"><Wallet className="w-3 h-3" />账户</label>
        ...
        {/* Notes */}
        <div>
          <label className="text-[11px] text-gray-400 mb-1.5 block">备注</label>
      ...
      {/* Footer */}
      <div className="p-3 border-t border-gray-50 flex items-center gap-2 shrink-0">
        {hasChanges ? (
          <>
            <button onClick={handleRevert} className="btn-secondary px-3 py-2 rounded-md text-[12px] flex items-center gap-1.5">
              <RotateCcw className="w-3.5 h-3.5 text-gray-500" />撤销
            </button>
          ...
          <button onClick={handleDelete} className="flex-1 py-2 rounded-md text-[12px] text-red-600 bg-red-50 hover:bg-red-100 transition-colors flex items-center justify-center gap-1.5">
